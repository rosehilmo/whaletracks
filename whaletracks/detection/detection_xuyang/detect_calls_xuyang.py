...
SLOPE_PATH = "slope_file.csv"#slope save path
# Constants
PLT_TIMESERIES = 1
PLT_SPECTROGRAM = 2
PLT_KERNEL = 3
...
def finding_slope(t,f,Sxx,tvec,fvec,BlueKernel,plotflag=True,scale_func=defaultScaleFunction,ylim=[12, 18]):
    ...
    """
    Sxx_log1=10*np.log10(Sxx)
    #print('The maximum of SXX is: ', np.max(Sxx_log1))
    #print('The minimum of SXX is: ', np.min(Sxx_log1))
    Sxx_log=Sxx_log1-np.min(Sxx_log1)
#Cross-correlate kernel with spectrogram
    ind1 = 0
    CorrVal = np.zeros(np.size(t) - (len(tvec)-1)) #preallocate array for correlation values
    ...
    #print(f)
    #print(Sxx_log.shape)
    detect_peaks, _ = find_peaks(CorrVal_scale, distance=300, height=(np.max(CorrVal_scale))/2)
    #print(t_scale[peaks])
    #t_peaks = t_scale[detect_peaks]
    #print(t_peaks)
    #print(np.diff(peaks))
#Visualize spectrogram and detection scores of of data  
...
#plot spectrogram on lower axis
        ...
        im.set_clim(85, np.max(Sxx_log))
        fig.colorbar(im, ax=ax1,orientation='horizontal')
        #ax1.plot(t_scale[peaks], Sxx_log[peaks], "x")
        ax1.set_xlim([t1, t2]) #look at spectrogram segment between given time boundaries
        ax1.set_ylim(ylim)
        ax1.set_ylabel('Frequency [Hz]')
        #ax1.set_xticks([])
        #ax1.set_xlabel('Time [seconds]')
        #print(len(f_call), f_call)
        plt.show()
    ...
    '''    
            for i in range(30,65):
                Sxx_call = Sxx_log[i]
                f_call = np.full(len(Sxx_call), f[i])
                peaks, _ = find_peaks(Sxx_call, distance=300, height=(np.max(Sxx_call))/2)
                ax1.plot(t_scale[peaks], f_call[peaks], "x")
                #print(sxx_call[peaks])
    '''
