    ...
    while utcend > utcstart_chunk:
        ...
        for idx in range(1, num_sta+1):
            ...
            if FINFLAG:
                ...
                snr_freqwidth = 3
                # Event metrics
                prominence = 5000  # 4 #.2 #.5 #min threshold   .1 for 0.3 second window
            ...
            [times, values] = detect.xcorr(
                t, f_sub, Sxx_sub, tvec, fvec, BlueKernel, plotflag=False, ylim=freqlim)
...
# Add freq info for blue whales
            if FINFLAG == False:
                ...
                [peak_freqs, start_freqs, end_freqs, peak_stds, start_stds, end_stds] = detect.freq_analysis(
                    analyzer_j, t, f_sub, Sxx_sub, uutcstart_chunk - .5*CHUNK_LENGTH)
            if FINFLAG:
            ...
            increment = int(timescount/2)

            # find number of minutes for range calculation
            #numchunks = round((utcend_chunk-utcstart_chunk)/60)
            #numchunks = round(((tr_filt.stats.endtime-tr_filt.stats.starttime)-CHUNK_LENGTH)/20)
            numchunks = len(analyzer_j.df)
            
            #import pdb; pdb.set_trace()

            for mp_iter in range(numchunks): #iterate for each call detected
                print(mp_iter)
                dettime_sec = analyzer_j.df.peak_time[mp_iter] - utc_times[0]
                timediff = np.array(times) - dettime_sec
                minind = np.argmin(abs(timediff))

                startind = minind - increment
                endind = minind + increment
                ...
                min_df_sub=analyzer_j.df.loc[(analyzer_j.df['peak_time'] >= (start_utc)) & 
                        (analyzer_j.df['peak_time'] < (end_utc))]
...
for site_ind in range(0,7): #range(13,len(sta_table['Sites'])):
    ...
    CHUNK_FILE = station+'_mp_Brydes_30s.csv'  
    DET_PATH = CHUNK_FILE
