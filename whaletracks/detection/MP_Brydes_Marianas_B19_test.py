...
CHUNK_FILE = "Marianas_Brydes_test_Oct16_10min.csv" #"AlaskaFins_LA21_chunk.csv" #Name of saved call file
...
DET_PATH="Marianas_Brydes_test_Oct16_10min.csv" #"AlaskaFins_LA21.csv" #Final save file
...
#STARTTIME = ("2013-01-18T00:00:00.000") #Mar amp test
#ENDTIME = ("2013-01-18T09:00:00.000")

#STARTTIME = ("2013-01-14T22:00:00.000") #Mar amp test #2
#ENDTIME = ("2013-01-15T12:00:00.000")

#STARTTIME = ("2012-09-23T12:00:00.000") #for testing on Brydes whales original september
#ENDTIME = ("2012-09-24T00:00:00.000")

STARTTIME = ("2012-10-16T00:00:00.000") #for testing on Brydes whales October maximu
...
def main(STARTTIME, ENDTIME,
    ...
    if os.path.isfile(CHUNK_FILE) and is_restart:
        ...
        auto_df_full = [pd.read_csv('auto_Marianas_Brydes_test_Oct16_10min.csv')]
        stack_df_full = [pd.read_csv('stack_Marianas_Brydes_test_Oct16_10min.csv')]
    else:
        analyzers = []
        auto_df_full = []
        stack_df_full = []
    ...
    while utcend > utcstart_chunk:
        ...
        for idx in range(1, num_sta+1):
            ...
            if FINFLAG:
                ...
                snr_freqwidth=5
                #Event metrics
                prominence= 50 #4 #.2 #.5 #min threshold   .1 for 0.3 second window
            ...
            if len(analyzer_j.df) >= 1: #if MP_FLAG is True
                ...
                [stacked_t,stacked_env,maxamps]=detect.stack_times(seconds,amplitude_envelope,utcstart_chunk,analyzer_j,3,2,pad_length = 8000) #get amplitudes in array
                #import pdb; pdb.set_trace()
            ...
            center_min = [utcstart_chunk+CHUNK_LENGTH/2-30, utcstart_chunk+CHUNK_LENGTH/2+30]


            min_df_sub=analyzer_j.df.loc[(analyzer_j.df['peak_time'] >= center_min[0]) & 
                        (analyzer_j.df['peak_time'] < center_min[1])]

            analyzers_chunk.append(min_df_sub)

            if MP_SPCT_FLAG and len(min_df_sub) >= 1: #if MP_FLAG is True
                
                #Stack detected calls by adding spectrograms together
                [tstack,fstack,Sxxstack] = detect.stack_spect(t,f_sub,Sxx_sub,utcstart_chunk,analyzer_j,dt_up,dt_down,pad_length = 8000)
                ...
                for det in range(0,len(analyzer_j.df)):
                    ...
                    mp_df = pd.concat([mp_df,mp_event])
                ...
                for det in range(0,len(analyzer_j.df)):
                    mp_event=analyzer_j.mp_picker(dettimes_chunk,autocorr_chunk, utcstart_chunk, dur=.1, prominence=0,distance=2,rel_height=.5)
                    #mp_df_auto=mp_df_auto.append(mp_event,ignore_index=True)
                    mp_df_auto= pd.concat([mp_df_auto,mp_event])
                ...
                #stack_df_full = pd.concat([stack_df_full,stack_df])
                #import pdb; pdb.set_trace()
                new_auto_df=pd.concat(auto_df_full)
                new_stack_df=pd.concat(stack_df_full)
                new_auto_df.to_csv('auto_Marianas_Brydes_test_Oct16_10min.csv', index=False)
                new_stack_df.to_csv('stack_Marianas_Brydes_test_Oct16_10min.csv', index=False)


                #Build array for peak displays
                timeinds=np.where((stacktimes > mp_event['arrival_1'][0]) & (stacktimes < mp_event['arrival_1'][0] + 10))
            ...
            if PLOTFLAG_STACK and len(min_df_sub) >= 1:  #plots stacked spectrogram
                ...
                fig.tight_layout()
                fig.savefig('Figures/Brydes/' + str(utcstart_chunk) + '.png', dpi=100)
                #plt.show()
                plt.close()
            #Calculate SNR info
        ...
        utcstart_chunk=utcstart_chunk+CHUNK_LENGTH/10
        utcend_chunk=utcend_chunk+CHUNK_LENGTH/10
        ...
        try:
            pickle.dump([datetimearray,ar,yax],open("peaks_Marianas_Brydes_test_stack_Oct16_10min.p","wb"))
        except:
            continue     
